...
window.AD_DEMO_CASES = [
  ...
  },
  {
    key: 'analyze',
    name: 'ROAS 분석 필요',
    cac: 30, tov: 120, budget: 250,   // 설정상 ROAS = 120/30 = 4.0
    note: 'ROAS는 안정권처럼 보이지만, 매출 대비 광고비가 높거나 제품 마진이 약 20%로 낮으면 실제로는 손해일 수 있습니다. ROAS 수치만으로는 손익을 판단하기 어려운 상황입니다.',
    // 순이익 = 매출×0.5(마진) − 매출×0.2 − 광고비 = 3600×0.3 − 900 = +$180 (−500~+250) · 실제 ROAS 4.0
    perf: { customers: 30, spend: 900, sales: 3600, startDaysAgo: 0 },
    answer: '제품 마진 자체를 올리는 것이 근본 해결입니다 (원가 절감·판매가 조정 등).\n마진을 올리기 어렵다면, 최소한 ROAS를 4.5배 이상으로 유지하세요.\n예) CAC $26 · TOV $120 → ROAS 4.6\n단, 광고는 끄지 말고 계속 모니터링하세요.',
    explain: 'ROAS를 4.5배 이상으로 올려도, 마진이 낮으면 광고비를 뺀 뒤 남는 이익은 여전히 빠듯할 수 있습니다.\n그래서 근본 해결은 제품 마진을 올리는 것(원가를 낮추거나 판매가를 조정)입니다. 그게 어렵다면 최소한 ROAS를 4.5배 이상으로 유지하며 원가·마진을 계속 모니터링해야 합니다.\n즉 ROAS 숫자가 아니라 "실제로 남는 이익"을 기준으로 판단하는 게 핵심입니다.',
    goal: {
      min: 4.5,
  ...
  },
  {
    ...
    note: '표의 Start 날짜를 보면 광고를 시작한 지 일주일이 지났습니다. 그런데도 주문이 2건뿐이라 매출이 매우 낮습니다. CAC가 너무 낮고 목표 객단가(TOV)가 너무 높은 설정이 원인입니다.',
